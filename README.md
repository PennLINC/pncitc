...
Then, using the code detailed in the `spin_test` folder here, Dr. Erica Baller performed the actual spin testing.  

### 8. Code added during revision

1. The histogram was created using R using `hist` - `hist(k,main="Distribution of k Values", xlab="k Values",col="blue", font=2, freq=TRUE, family="times")`. A similar code was used for the log k values.
...
4. Finally, to address comments from reviewers, we re-ran the original model with the covariates `cog1` and `cog2` sourced from `n293_psych_cog.csv` as before. These were an overall efficiency and accuracy measure from CNBs, respectively. This meant re-running the MDMR with the formula `logk+relMeanRMSmotion+sex+age+cog1+cog2` in the folder `/cbica/projects/pncitc/ignore/logk_motion_sex_age_cog`.
----------------------------------------------------------------------------------------------------------------------------------------------------------
